...
type IClient interface {
	...
	GetBlockReceipts(hash common.Hash) (ethTypes.Receipts, error) // not implemented
...
func (cl *Client) GetBlockReceipts(hash common.Hash) (ethTypes.Receipts, error) {
	...
	if hb.GasUsed() == uint64(0) || len(hb.Transactions()) == 0 {
		return nil, nil
	}
	...
	for _, txh := range txhs {
		qch <- &rcq{txh.Hash().String(), nil, nil, RPCCallRetry}
	}
	...
	for _, txh := range txhs {
		if r, ok := rmap[txh.Hash().String()]; ok {
