...
func WriteE2EConifg(path, ethNodeURL, baseNodeURL, aleoNodeURL string, ethStartHeight, baseStartHeight, aleoStartSeqNumber uint64, benchmark bool) {
	...
	relayConfig := &Config{
		...
		ChainConfigs: []*ChainConfig{
			{
				...
				DestChains: map[string]DurationConfig{
					"ethereum": {
						PacketValidityWaitDuration: time.Minute * 5,
						FinalityHeight:             100,
					},
					"base": {
						PacketValidityWaitDuration: time.Minute * 4,
						FinalityHeight:             80,
					},
				},
		...
		CollectorServiceConfig: CollecterServiceConfig{
			...
			CaCertificate:       "/configs/.mtls/ca.cer",
			AttestorCertificate: "/configs/.mtls/attestor1.crt",
			AttestorKey:         "/configs/.mtls/attestor1.key",
		},
	}
