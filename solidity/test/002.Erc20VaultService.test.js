...
describe("Erc20VaultService", () => {
  let deployer,
    ...
    usdcMock,
    other,
    Erc20VaultServiceImpl,
    erc20VaultServiceInstance,
    Proxy,
    initializeData,
    Erc20VaultServiceProxy;
  let abi;
  ...
  beforeEach(async () => {
    ...
    Erc20VaultServiceImpl = await ethers.getContractFactory(
      "Erc20VaultService"
    );
    ...
    initializeData = new ethers.utils.Interface(abi).encodeFunctionData(
      "Erc20VaultService_init",
      [usdcMock.address, "USDC Vault", owner.address]
    );
    const proxy = await Proxy.deploy(
      erc20VaultServiceInstance.address,
      initializeData
    );
  ...
  it("should not initialize if token address is zero", async () => {
    initializeData = new ethers.utils.Interface(abi).encodeFunctionData(
      "Erc20VaultService_init",
      [ethers.constants.AddressZero, "USDC", owner.address]
    );
    await expect(
      Proxy.deploy(erc20VaultServiceInstance.address, initializeData)
    ).to.be.revertedWith("Erc20VaultService: only erc20 address");
  });
  ...
  it("reverts if the contract is already initialized", async function () {
    await expect(
      Erc20VaultServiceProxy["Erc20VaultService_init(address,string,address)"](
        usdcMock.address,
        "USDC",
        owner.address
      )
    ).to.be.revertedWithCustomError(Erc20VaultServiceProxy,"InvalidInitialization");
  });

  it("should not transfer if caller is not admin", async () => {
    await expect(
      Erc20VaultServiceProxy.connect(other).transfer(1000)
    ).to.be.revertedWithCustomError(Erc20VaultServiceProxy,"OwnableUnauthorizedAccount");
  });

  it("should not transfer if balance is less than send amount", async () => {
    // await Erc20VaultServiceProxy.connect(owner).transfer(500);

    // Attempt to transfer more than the balance
    await expect(
      Erc20VaultServiceProxy.connect(owner).transfer(1000)
    ).to.be.revertedWithCustomError(usdcMock, 'ERC20InsufficientBalance').withArgs(Erc20VaultServiceProxy.address, 0, 1000);
  });
  ...
  it("should not transfer if sender is blacklisted", async () => {
    ...
    await expect(
      Erc20VaultServiceProxy.connect(owner).transfer(10)
    ).to.be.revertedWithCustomError(Erc20VaultServiceProxy,"SafeERC20FailedOperation");
  });
...
describe("Erc20VaultService Upgradeability", () => {
  ...
  beforeEach(async () => {
    ...
    initializeData = new ethers.utils.Interface(abi).encodeFunctionData(
      "Erc20VaultService_init",
      [usdcMock.address, "USDC Vault 2", owner.address]
    );
    const proxy = await Proxy.deploy(
      erc20VaultServiceInstance.address,
      initializeData
    ...
    Erc20VaultServiceV2 = await ethers.getContractFactory(
      "Erc20VaultServiceV2"
    );
    ...
    upgradeData = new ethers.utils.Interface(
      Erc20VaultServiceV2ABI
    ).encodeFunctionData("initializev2", [5]);
    await Erc20VaultServiceProxy.connect(owner).upgradeToAndCall(
      erc20VaultServiceV2Instance.address,
      upgradeData
    );
  ...
  it("only owner should be able to upgrade", async () => {
    await expect(
      Erc20VaultServiceProxy.connect(other).upgradeToAndCall(
        erc20VaultServiceV2Instance.address,
        upgradeData
      )
    ).to.be.reverted;
  ...
  it("reverts if the contract is initialized twice", async function () {
    await expect(Erc20VaultServiceProxy.initializev2(100)).to.be.revertedWithCustomError(
      Erc20VaultServiceProxy,
