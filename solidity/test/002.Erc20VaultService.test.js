...
describe("Erc20VaultService Upgradeability", () => {
    ...
    beforeEach(async () => {
        ...
        initializeData = new ethers.utils.Interface(abi).encodeFunctionData("Erc20VaultService_init", [usdcMock.address, "USDC Vault 2", owner.address]);
        const proxy = await Proxy.deploy(erc20VaultServiceInstance.address, initializeData);
        await proxy.deployed();
        ...
        await Erc20VaultServiceProxy.connect(owner).upgradeToAndCall(erc20VaultServiceV2Instance.address, upgradeData);
