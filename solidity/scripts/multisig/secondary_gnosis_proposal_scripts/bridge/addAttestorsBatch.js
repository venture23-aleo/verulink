...
async function proposeAddAttestorsBatchTransaction(signer) {
  ...
  const attestors = [
    process.env.ATTESTOR1,
    process.env.ATTESTOR2,
    process.env.ATTESTOR3,
...
(async () => {
  try {
    ...
    const safeTxHash = await proposeAddAttestorsBatchTransaction(deployerSigner);
    ...
    console.log("Attestors added successfully!");
  } catch (error) {
