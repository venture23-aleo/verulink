...
async function proposeRemoveTokenserviceTransaction(signer) {
  ...
  const tokenservicetoremove = process.env.TOKENSERVICEPROXY_ADDRESS;
  ...
  const calldata = iface.encodeFunctionData("addTokenService", [tokenservicetoremove]);
...
(async () => {
  try {
    ...
    const safeTxHash = await proposeRemoveTokenserviceTransaction(deployerSigner);
    ...
    console.log("TokenService removed successfully!");
  } catch (error) {
