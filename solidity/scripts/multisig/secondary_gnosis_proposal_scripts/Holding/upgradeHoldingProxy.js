...
async function proposeUpgradeTransaction(signer) {
    ...
    const holdingnewImplementationAddress = process.env.HOLDINGNEWIMPLEMENTATION_ADDRESS;
    const HoldingProxyAddress = process.env.HOLDINGPROXY_ADDRESS;

    // Encode the function call for upgrading the contract
    const deployerInterface = new ethers.utils.Interface(HoldingImpl.interface.format());
    const deployCallData = deployerInterface.encodeFunctionData("upgradeTo", [
        holdingnewImplementationAddress
    ]);
...
(async () => {
    try {
        ...
        console.log("Upgrade of Holding successful!");
    } catch (error) {
