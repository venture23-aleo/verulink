...
describe("Token Bridge ", () => {
  ...
  describe.skip("Governance", () => {
    describe("Pausability", () => {
      test("should not unpause by non-owner", async () => {
        ...
        // todo checked mapping of previous value
      ...
      test("should not pause by non-owner", async () => {
        ...
        // todo checked mapping of paused value
        expect(await bridge.bridge_settings(BRIDGE_PAUSABILITY_INDEX)).toBe(BRIDGE_UNPAUSED_VALUE);
        await expect(tx.wait()).rejects.toThrow();
      }, TIMEOUT);
    ...
    describe("Update threshold", () => {
      ...
      test("Non-owner cannot update threshold", async () => {
        ...
        let current_threshold_index: number = await bridge.bridge_settings(BRIDGE_THRESHOLD_INDEX);
        // todo checked mapping of threshold
    ...
    describe("Add/Remove Chain", () => {
      ...
      describe("Remove Chain", () => {
        ...
        test("should have a chain to remove it", async () => {
          ...
          await expect(tx.wait()).rejects.toThrow();
          // todo again check mapping of chain
        },
          TIMEOUT
        );
  ...
  describe.skip("Consume", () => {
    ...
    test.skip.failing("From non-supported service, transaction should be failed and undefined", async () => {
    ...
    test.skip("Screening failed due to not signing packets", async () => {
      const packet = createPacket(aleoUser1, BigInt(1100_000), aleoTsAddr, ethTsContractAddr, ethChainId);
      let packetId: PacketId = {
        chain_id: packet.source.chain_id,
        sequence: packet.sequence
      }
  ...
  describe.skip("Transfer Ownership", () => {
    ...
    test("should not transfer ownership by non-admin", async () => {
      ...
      // TODO: checked mapping owner should be previous owner 
