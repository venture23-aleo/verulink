...
import { AddMember, InPacketFull, ProposalSign, RemoveMember, TbAddAttestor, TbDisableChain, TbDisableService, TbEnableChain, TbEnableService, TbRemoveAttestor, TbUpdateThreshold, TsRemoveChain, TsRemoveToken, TsSupportChain, TsSupportToken, TsUpdateMinimumTransfer, TsUpdateOutgoingPercentage, UpdateThreshold } from "../artifacts/js/types";
import { THRESHOLD_INDEX, TOTAL_ATTESTORS_INDEX, TOTAL_MEMBERS_INDEX, TOTAL_PROPOSALS_INDEX, aleoTsContract, aleoUser1, aleoUser2, aleoUser3, aleoUser4, aleoUser5, aleoUser6, councilProgram, ethChainId, ethTsContract, highThreshold, lowThreshold, newThreshold, normalThreshold, nullError, nullError2, nullError3, outgoing_percentage_in_time, wusdcConnectorAddr, wusdcTokenAddr } from "./mockData";
...
const council_fromNonMember = new CouncilContract({mode: "execute", networkName: "testnet3",privateKey: "APrivateKey1zkpEhkVqeyXJuVLhfpwM1fmHZzHBAz8q1fqoC7ihD89QdGR",});
const council_fromAnotherMember = new CouncilContract({mode: "execute", networkName: "testnet3",privateKey: "APrivateKey1zkp2RWGDcde3efb89rjhME1VYA8QMxcxep5DShNBR6n8Yjh"});
const bridge = new Token_bridgeContract({mode: "execute"});
const token_service = new Token_serviceContract({mode: "execute"});
const wudc_connector = new Wusdc_connectorContract({mode: "execute"});
const wudc_holding = new Wusdc_holdingContract({mode: "execute"});
const wudc_token = new Wusdc_tokenContract({mode: "execute"});
...
describe("Council", () => {
    ...
    describe("Propose", () => {
        test.skip("should not propose from other members", async() => {
            ...
            const addNewProposalHash = hashStruct(js2leo.getAddMemberLeo(addNewMemeberProposal));
        ...
        test("proposal id must be correct", async () => {
            proposalId = parseInt((await council.proposals(TOTAL_PROPOSALS_INDEX)).toString());
            const addNewMemeberProposal : AddMember = {
                      id: proposalId,
                      new_member: admin,
                      new_threshold: normalThreshold
        ...
        test("should add a proposal",async () => {
            ...
            const addNewProposalSignHash = hashStruct(js2leo.getProposalSignLeo(addNewProposalSign));
            tx =  await council.propose(proposalId, addNewProposalHash);
            await tx.wait();

            expect(await council.proposals(TOTAL_PROPOSALS_INDEX)).toBe(BigInt(addNewMemeberProposal.id));
            expect(await council.proposals(addNewMemeberProposal.id)).toBe(addNewProposalHash);
            expect(await council.proposal_vote_signs(addNewProposalSignHash)).toBe(true);
    ...
    test("should execute a proposal to add new member", async () => {
        ...
        const removeProposalHash = hashStruct(js2leo.getRemoveMemberLeo(removeMemeberProposal));
        tx = await council.propose(proposalId, removeProposalHash);
        await tx.wait();

        // executing to remove member3 and checking mappings
        tx = await council.remove_member(removeMemeberProposal.id, removeMemeberProposal.existing_member, removeMemeberProposal.new_threshold);
        await tx.wait();
        try{
    ...
    test("should update the threshold and vote", async () => {
      ...
      const updateThresholdHash =  hashStruct(js2leo.getUpdateThresholdLeo(updateThreshold));
      const updateThresholdSign : ProposalSign = {
        proposal: updateThresholdHash,
        member: aleoUser2
      }
      const updateThresholdSignHash = hashStruct(js2leo.getProposalSignLeo(updateThresholdSign));
      // propse to update threshold
      tx = await council.propose(proposalId, updateThresholdHash);
      await tx.wait();
      // vote to update threshold
      tx = await council_fromAnotherMember.vote(updateThresholdHash, normalThreshold);
      await tx.wait();
      expect(await council.proposal_vote_signs(updateThresholdSignHash)).toBeTruthy();
      expect(await council.proposal_vote_counts(updateThresholdHash)).toBe(2);
      // execute update threshold
      tx = await council.update_threshold(updateThreshold.id, updateThreshold.new_threshold);
      await tx.wait();
    ...
    describe("Call to external programs", () => {
        ...
        test("should add attestor", async() => {
            ...
            tx = await council.tb_add_attestor(proposalId, councilProgram, normalThreshold);
            await tx.wait();

            expect(await council.proposal_executed(addAttestorHash)).toBe(true);
            expect(await bridge.attestors(councilProgram)).toBe(true);
            expect(await bridge.attestor_settings(THRESHOLD_INDEX)).toBe(normalThreshold);
        ...
        test("should remove attestor", async() => {
            ...
            const removeAttestorHash = hashStruct(js2leo.getTbRemoveAttestorLeo(removeAttestor));
            //propose to remove attestor
            tx = await council.propose(proposalId, removeAttestorHash);
            await tx.wait();

            // execute to remove attestor
            tx = await council.tb_remove_attestor(proposalId, councilProgram, normalThreshold);
            await tx.wait();
    ...
    test("should update token bridge threshold", async() => {
            ...
            const tokenBridge_threshold_hash = hashStruct(js2leo.getTbUpdateThresholdLeo(tokenBridge_threshold));
            tx = await council.propose(tokenBridge_threshold.id, tokenBridge_threshold_hash);
            await tx.wait();
            tx = await council.tb_update_threshold(tokenBridge_threshold.id, tokenBridge_threshold.new_threshold);
            await tx.wait();
            expect(await council.proposal_executed(tokenBridge_threshold_hash)).toBe(true);
    ...
    test("should enable chain bridge", async() =>{
            ...
            const approveChainProposalHash = hashStruct(js2leo.getTbEnableChainLeo(approveChainProposal));
            ...
            const approveChainProposalsHash = hashStruct(js2leo.getTbEnableChainLeo(approveChainProposals));
            expect(await council.proposal_executed(approveChainProposalsHash)).toBe(true);
    ...
    test("should disable chain bridge", async () => {
      ...
      const disapproveChainProposalHash = hashStruct(js2leo.getTbDisableChainLeo(disapproveChainProposal));
      ...
      const disapproveChainProposalsHash = hashStruct(js2leo.getTbDisableChainLeo(disapproveChainsProposal));
      tx = await council.disapprove_chain_bridge(
        disapproveChainProposal.id,
        disapproveChainProposal.chain_id,
      );
      await tx.wait();
      expect(await council.proposal_executed(disapproveChainProposalsHash)).toBe(true);
      let errorMsg = "";
      try{
    ...
    test("should enable token_service program", async() =>{
      ...
      const enableServiceProposalHash = hashStruct(js2leo.getTbEnableServiceLeo(enableServiceProposal));
      ...
      const enableServiceProposalsHash = hashStruct(js2leo.getTbEnableServiceLeo(enableServicesProposal));
      expect(await council.proposal_executed(enableServiceProposalsHash)).toBe(true);
    ...
    test("should disable token_service program", async() =>{
      ...
      const disableServiceProposalHash = hashStruct(js2leo.getTbDisableServiceLeo(disableServiceProposal));
      ...
      await tx.wait();
      expect(await council.proposal_executed(disableServiceProposalHash)).toBe(true);
      let errorMsg = "";
      try{
      ...
      expect(errorMsg).toBe(nullError);
        ...
        //     test.todo("Update Governance")
            test("should support chain token service", async() =>{
                ...
                const approveChainProposalHash = hashStruct(js2leo.getTsSupportChainLeo(approveChainProposal));
                tx = await council.propose(proposalId, approveChainProposalHash);
                await tx.wait();
                ...
                await tx.wait();
                expect(await council.proposal_executed(approveChainProposalHash)).toBe(true);
                expect(await token_service.token_service_contracts(ethChainId)).toStrictEqual(approveChainProposal.token_service);
            }, 20000_000);
        ...
        test("should support chain token service", async () => {
            ...
            const disapproveChainProposalHash = hashStruct(js2leo.getTsRemoveChainLeo(disapproveChainProposal));
            tx = await council.propose(proposalId, disapproveChainProposalHash);
            await tx.wait();
            ...
            await tx.wait();
            expect(await council.proposal_executed(disapproveChainProposalHash)).toBe(true);
            let errorMsg = "";
            try{
            ...
            expect(await token_service.max_outgoing_percentage(wusdcTokenAddr)).toStrictEqual(outgoing_percentage_in_time);
            expect(await token_service.minimum_transfers(wusdcTokenAddr)).toBe(tsSupportToken.minimum_transfer);
            expect(await token_service.token_connectors(wusdcTokenAddr)).toBe(wusdcConnectorAddr);
        }, 20000_000);
        ...
        test("should update minimum transfer", async() => {
            ...
            const TsUpdateMinimumTransferHash = hashStruct(js2leo.getTsUpdateMinimumTransferLeo(TsUpdateMinimumTransfer));
            tx = await council.propose(proposalId, TsUpdateMinimumTransferHash);
            await tx.wait();
            tx = await council.ts_update_minimum_transfer(TsUpdateMinimumTransfer.id, TsUpdateMinimumTransfer.token_id, TsUpdateMinimumTransfer.minimum_transfer, 200, 1);
            await tx.wait();
            expect(await council.proposal_executed(TsUpdateMinimumTransferHash)).toBeTruthy();
            expect(await token_service.minimum_transfers(wusdcTokenAddr)).toBe(BigInt(200));
        }, 20000_000);
        ...
        test("should update Outgoing Percentage", async() => {
            ...
            const TsUpdateOutgoingHash = hashStruct(js2leo.getTsUpdateOutgoingPercentageLeo(TsUpdateOutgoing));
            tx = await council.propose(proposalId, TsUpdateOutgoingHash);
            await tx.wait();
            tx = await council.ts_update_outgoing_percentage(TsUpdateOutgoing.id, TsUpdateOutgoing.token_id, TsUpdateOutgoing.outgoing_percentage, TsUpdateOutgoing.timeframe);
            await tx.wait();
            expect(await council.proposal_executed(TsUpdateOutgoingHash)).toBeTruthy();
            const new_outgoing_percentage = {"outgoing_percentage": 200, "timeframe": 1}
            expect(await token_service.max_outgoing_percentage(wusdcTokenAddr)).toStrictEqual(new_outgoing_percentage);
        }, 20000_000);
        ...
        test("should remove a token", async() => {
            ...
            const RemoveTokenHash = hashStruct(js2leo.getTsRemoveTokenLeo(RemoveToken));
